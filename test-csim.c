             ...
             int *test_hits, int *test_misses, int *test_evictions) /* out */
{
  ...
  sprintf(cmd, "./csim-ref -s %d -E %d -b %d -t %s > /dev/null", s, E, b,
          trace);
  ...
  if (!fp) {
    fprintf(stderr, "Error: Results for reference simulator not found. Use the "
  ...
  sprintf(cmd, "./csim -s %d -E %d -b %d -t %s > /dev/null", s, E, b, trace);
  system("rm -rf .csim_results");
  status = system(cmd);
  if (status == -1) {
  ...
  fp = fopen(".csim_results", "r");
  if (!fp) {
    fprintf(stderr, "Error: Results for test simulator not found. Use the "
                    "printSummary() function\n");
    return 0;
  }
...
void test_csim() {
  ...
  char *trace[N] = {"traces/yi2.trace",   "traces/yi.trace",
                    "traces/dave.trace",  "traces/trans.trace",
  ...
  for (i = 0; i < N; i++) {
    ...
    /*printf("Testing (s,E,b)=(%d,%d,%d) on file %s\n",
      s[i], E[i], b[i], trace[i]);*/
    status[i] = runtrace(s[i], E[i], b[i], trace[i], &ref_hits[i],
                         &ref_misses[i], &ref_evictions[i], &test_hits[i],
                         &test_misses[i], &test_evictions[i]);
  ...
  for (i = 0; i < N; i++) {
    ...
    if (status[i] != 0 && ref_hits[i] != -1 && ref_misses[i] != -1 &&
        ref_evictions[i] != -1 && test_hits[i] != -1 && test_misses[i] != -1 &&
        test_evictions[i] != -1) {
